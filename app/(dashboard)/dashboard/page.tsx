  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 lg:py-10 space-y-6 lg:space-y-8">
        ...
        {profile && metrics && (
          <div className="space-y-4">
            <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Profile Performance</h2>
            <DriverMetrics
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex-1 flex flex-col">
              <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[10px] font-black text-[#0B1F3D] uppercase tracking-[0.2em]">Business Profile</h2>
                </div>
          ...
          <div className="lg:col-span-4 space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 lg:p-8">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Account</h2>
              </div>
            ...
            <div className="bg-[#0B1F3D] rounded-3xl p-6 lg:p-8 text-white shadow-2xl relative overflow-hidden group">
              ...
              <div className="mt-6 pt-6 border-t border-white/10 flex items-center justify-between text-[10px] font-black text-white/30 tracking-[0.2em] uppercase">
...
function QuickAction({ href, title, icon: Icon }: { href: string; title: string; icon: any }) {
  return (
    <Link
      href={href}
      className="flex items-center justify-between gap-3 p-3.5 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all group"
    >
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0">
          <Icon className="w-4 h-4 text-amber-400" />
        </div>
        <span className="font-bold text-xs tracking-tight">{title}</span>
      </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="relative w-28 h-28 rounded-3xl overflow-hidden border-4 border-white bg-white shadow-lg">
          {profile.profileImageUrl ? (
            <Image src={profile.profileImageUrl} alt={profile.displayName} fill className="object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-3xl text-[#0B1F3D] font-bold bg-gray-50">
              {profile.displayName.charAt(0)}
            </div>
          )}
        </div>
        <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${statusColor}`}>
          {profile.status}
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 space-y-4 text-center sm:text-left">
        <div>
          <h3 className="text-2xl font-black text-gray-900 tracking-tight mb-1">{profile.displayName}</h3>
          <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 text-gray-500 font-bold text-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 pt-4 border-t border-gray-50 text-xs">
        ...
        <div className="pt-4">
          <Link
            href="/dashboard/profile"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gray-100 text-[#0B1F3D] font-black text-xs hover:bg-gray-200 transition-all transform active:scale-95 shadow-sm"
          >
            Edit full profile
